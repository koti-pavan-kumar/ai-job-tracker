...
# Initialize Database Schema Structures - Permanent Setup
...
app.add_middleware(
    ...
    allow_origins=[
        "http://localhost:5173", 
        "https://ai-job-tracker-frontend-2fj1.onrender.com"
    ],
...
# ─── 100% WORKING CLEAN REGISTER ENDPOINT ───
@app.post("/register")
def register_user(user_data: schemas.UserCreate, db: Session = Depends(database.get_db)):
    ...
    new_user = models.User(
        ...
        phone=getattr(user_data, "phone", "") if getattr(user_data, "phone", "") else f"no_phone_{user_data.username}",
...
# ─── SINGLE BULLETPROOF LOGIN ENDPOINT ───
@app.post("/token")
...
def get_current_user(token: str = Depends(oauth2_scheme)):
    try:
        ...
        if username is None:
            raise HTTPException(status_code=401, detail="Token validation collapsed.")
        return username
    except JWTError:
...
def analyze_url(request: schemas.URLIngestRequest):
    ...
    if not scraped_data or scraped_data.get("company_name") == "Error Parsing":
        raise HTTPException(status_code=400, detail="Unable to parse details from URL.")
...
def update_job(job_id: int, job_update: schemas.JobUpdate, db: Session = Depends(database.get_db), current_user: str = Depends(get_current_user)):
...
def generate_assets(request: schemas.AssetGenerationRequest, db: Session = Depends(database.get_db), current_user: str = Depends(get_current_user)):
    db_job = get_user_job_or_raise(request.job_id, db, current_user)
    jd_payload = {"job_title": db_job.job_title, "company_name": db_job.company_name, "raw_jd": db_job.raw_jd}
...
async def upload_resume_and_tailor(job_id: int = Form(...), asset_type: str = Form(...), file: UploadFile = File(...), db: Session = Depends(database.get_db), current_user: str = Depends(get_current_user)):
    contents = await file.read()
    if contents.startswith(b'\xff\xd8\xff'):
        raise HTTPException(status_code=400, detail="The uploaded file appears to be an IMAGE. Please upload a document.")
...
def download_job_pdf(job_id: int, asset_type: str = "resume", db: Session = Depends(database.get_db), current_user: str = Depends(get_current_user)):
    db_job = get_user_job_or_raise(job_id, db, current_user)
    text_content = db_job.tailored_resume if asset_type == "resume" else db_job.tailored_cover_letter
    if not text_content:
        raise HTTPException(status_code=404, detail="No content found to download yet.")
    ...
    for line in text_content.replace("**", "").replace("*", "").split("\n"):
        if not line.strip():
    ...
    doc.build(story)
    buffer.seek(0)
    return StreamingResponse(buffer, media_type="application/pdf", headers={"Content-Disposition": f"attachment; filename=asset.pdf"})

@app.get("/jobs/{job_id}/download-docx")
def download_job_docx(job_id: int, asset_type: str = "resume", db: Session = Depends(database.get_db), current_user: str = Depends(get_current_user)):
    db_job = get_user_job_or_raise(job_id, db, current_user)
    text_content = db_job.tailored_resume if asset_type == "resume" else db_job.tailored_cover_letter
    if not text_content:
...
def forgot_password_reset(payload: schemas.PasswordResetRequest, db: Session = Depends(database.get_db)):
    ...
    if not user:
        raise HTTPException(status_code=404, detail="No workspace profile matches this identifier.")
...
def get_all_users(db: Session = Depends(database.get_db), current_username: str = Depends(get_current_user)):
    ...
    return [{"id": u.id, "username": u.username, "name": getattr(u, "name", ""), "email": getattr(u, "email", ""), "is_admin": getattr(u, "is_admin", False)} for u in users]
...
def admin_delete_user(user_id: int, db: Session = Depends(database.get_db), current_username: str = Depends(get_current_user)):
    ...
    if not user_to_delete:
        raise HTTPException(status_code=404, detail="Target user missing.")
    if user_to_delete.id == user.id:
        raise HTTPException(status_code=400, detail="Administrative profile self-deletion blocked.")

    db.query(models.JobApplication).filter(models.JobApplication.user_id == user_id).delete()
    db.delete(user_to_delete)
    db.commit()
    return {"message": "User account purged successfully."}
